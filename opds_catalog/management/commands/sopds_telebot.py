...
class Command(BaseCommand):
    ...
    def startCommand(self, update: Update, context: CallbackContext):
        context.bot.sendMessage(chat_id=update.message.chat_id, text=_(
    ...
    def BookPager(self, books, page_num, query):
        ...
        buttons = [InlineKeyboardButton('1 <<', callback_data='%s%s%s' % (query, query_delimiter, 1)),
                   InlineKeyboardButton('%s <' % op.previous_page_number,
                                        callback_data='%s%s%s' % (query, query_delimiter, op.previous_page_number)),
                   InlineKeyboardButton('[ %s ]' % op.number,
                                        callback_data='%s%s%s' % (query, query_delimiter, 'current')),
                   InlineKeyboardButton('> %s' % op.next_page_number,
    ...
    def getBooks(self, update: Update, context: CallbackContext):
        ...
        context.bot.send_message(chat_id=update.message.chat_id, text=response['message'], parse_mode='HTML',
    ...
    def getBooksPage(self, update: Update, context: CallbackContext):
        ...
        context.bot.edit_message_text(chat_id=callback_query.message.chat_id,
                                      message_id=callback_query.message.message_id, text=response['message'],
                                      parse_mode='HTML', reply_markup=response['buttons'])
        return
